...
@Directive({
  selector: '[ultPopoverTriggerFor]',
  exportAs: 'ultPopoverTriggerFor',
  host: {
    class: 'ult-popover-trigger-for',
    '[class.ult-popover-trigger-for--is-open]': 'api.isOpen()',
  }
})
export class PopoverTriggerForDirective implements  OnDestroy {
  @Input('ultPopoverTriggerFor') popoverTemplateRef!: TemplateRef<unknown>;
  @Input() trigger: PopoverTrigger = 'click';
  @Input() position: PopoverPosition = 'below-center';
  @Input() delay = 500;
  @Input() origin: FlexibleConnectedPositionStrategyOrigin;
