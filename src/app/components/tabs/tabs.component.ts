import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';
import { UltTabsModule } from '@ultima-ui/components';
import { UltBadgeModule, UltFormsModule, UltRadioModule, UltIconModule } from '@ultima-ui/components';
import { InlineCodeDirective } from '../../_libraries/inline-code.directive';
import { CodeBlockComponent } from '../../_libraries/code-block/code-block.component';
import { CodeExampleModule } from '../../_libraries/code-example/code-example.module';
import { PlaygroundModule } from '../../_libraries/playground/playground.module';

@Component({
  standalone: true,
  imports: [
    CommonModule,
    UltTabsModule,
    UltBadgeModule,
    InlineCodeDirective,
    CodeBlockComponent,
    CodeExampleModule,
    PlaygroundModule,
    UltFormsModule,
    UltRadioModule,
    UltIconModule
  ],
  templateUrl: './tabs.component.html'
})
export class TabsComponent {
  importModule = `import { UltTabsModule } from '@ultima-ui/components';`;
  align = 'center';
}
